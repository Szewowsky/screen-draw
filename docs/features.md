...
| Text | `X` | Click to place a focused single-line text caret and type immediately. `Enter` commits and `Esc` cancels. Non-empty text also commits when you click another text point, switch tools, pin the annotations, fully exit drawing, or blur the input; an empty caret cancels. While the caret is open, `⌘Z`/`⌘⇧Z` edit the text instead of undoing the canvas. Committed text has the same forgiving hit area as other shapes, and is selectable, movable, restylable, deletable, and undoable. |
