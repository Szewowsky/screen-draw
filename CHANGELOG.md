...
- **Spotlight effect.** The spotlight presenter effect now dims the screen outside a feathered cursor-centered focus area, with Settings controls for radius and dim amount.
- **Annotated screenshot export.** Press `D` or the toolbar camera button to save the active display with committed annotations to `~/Downloads` and copy the PNG to the clipboard.
